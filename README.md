...
**Key features:**
- CPU, memory, load average time-series charts
- Docker container status tracking
- Port open/closed monitoring
- Discord alert notifications (CPU / memory thresholds)
...
All endpoints require `Authorization: Bearer <JWT or WEB_API_KEY>`.
...
| `GET` | `/api/stream?key=<WEB_API_KEY>` | SSE stream (`metrics` + `status` events) |

---

## Database Schema

**`metrics`** — TimescaleDB hypertable, 90-day retention, 1-day chunk interval
**`hosts`** — Agent registry (scrape interval, thresholds, monitored ports/containers)
**`alert_configs`** — Alert rules; `NULL host_key` = global default, per-host rows override
...
|---|---|
| Backend | Rust, Axum 0.8, sqlx 0.8, TimescaleDB |
| Frontend | Next.js 16, React 19, Recharts, SWR, next-intl |
| Agent | Rust, tokio, sysinfo, bollard (Docker) |
